...
import { SMSMockService } from '../src/modules/notifications/services/sms-mock.service'
...
async function testSMSMockService() {
  console.log('🚀 Testing SMS Mock Service...\n')
  
  // Test 1: SMS without Twilio credentials (mock mode)
  console.log('📱 Test 1: SMS sin credenciales Twilio (modo mock)')
  try {
    const smsService = new SMSMockService(mockConfigService)
  ...
  // Test 5: Bulk SMS with realistic failure simulation
  console.log('\n📱 Test 5: SMS masivos (bulk) con fallos simulados')
  try {
    const smsService = new SMSMockService(mockConfigService)
    const recipients = [
      '+1234567890',
  ...
  try {
    const smsService = new SMSMockService(mockConfigServiceWithTwilio)
  ...
  console.log('\n🎉 ¡Pruebas de SMS Mock completadas!')
}

// Run the tests
testSMSMockService().catch(console.error)